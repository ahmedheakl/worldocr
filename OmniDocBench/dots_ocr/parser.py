...
class DotsVLProcessor(Qwen2_5_VLProcessor):
    # Make only the pieces you actually need required:
    attributes = ["image_processor", "tokenizer"]
    # Optional: make it explicit we don't expect a video processor
    video_processor_class = None

    def __init__(self, image_processor=None, tokenizer=None, chat_template=None, **kwargs):
        # Set your custom image token/id first
        self.image_token = "<|imgpad|>" if not hasattr(tokenizer, "image_token") else tokenizer.image_token
        self.image_token_id = 151665 if not hasattr(tokenizer, "image_token_id") else tokenizer.image_token_id

        # Add video token attributes (required by parent class even if not used)
        self.video_token = "<|video_pad|>" if not hasattr(tokenizer, "video_token") else getattr(tokenizer, "video_token", None)
        self.video_token_id = 151656 if not hasattr(tokenizer, "video_token_id") else getattr(tokenizer, "video_token_id", None)

        # Initialize ProcessorMixin with ONLY the required attributes
...
class DotsOCRParser:
    ...
    def _load_hf_model(self):
        ...
        model_path = "../data/weights/DotsOCR"
        ...
        self.model.eval()
        self.processor = DotsVLProcessor.from_pretrained(model_path, trust_remote_code=True,use_fast=True)
    ...
    def _inference_with_hf(self, image, prompt):
        ...
        generated_ids = self.model.generate(**inputs, max_new_tokens=24000, use_cache=True)
