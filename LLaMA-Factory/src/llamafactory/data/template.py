...
# Behavior:
# - Each message is prefixed with: <|start_of_role|>{role}<|end_of_role|>
# - Message content follows immediately
# - Each message ends with: <|end_of_text|>\n
# - After a user message, we open the assistant role header to prompt generation
register_template(
    name="smoldocling",
    format_user=StringFormatter(
        slots=[
    ...
    mm_plugin=get_mm_plugin(name="smoldocling", image_token="<image>"),
...
register_template(
    ...
    # If your Granite-Docling checkpoint is VLM-capable, keep mm_plugin with <image>.
    # If it's text-only, you can safely remove mm_plugin.
    mm_plugin=get_mm_plugin(name="granite_docling", image_token="<image>"),
    stop_words=["<|end_of_text|>"],
)
