...
class GraniteDoclingPlugin(BasePlugin):
    @override
    def process_messages(
        self,
        messages: list[dict[str, str]],
        ...
        # Text-only or image-only; no video/audio in the Granite-Docling chat-template
        self._validate_input(processor, images, videos, audios)
        self._validate_messages(messages, images, videos, audios)

        msgs = deepcopy(messages)
        for msg in msgs:
            # Just map the framework placeholder to the model's image special token, no expansion.
            msg["content"] = msg["content"].replace(IMAGE_PLACEHOLDER, self.image_token)
        return msgs
    
class SmolDoclingPlugin(BasePlugin):
    @override
    def process_messages(
        self,
        messages: list[dict[str, str]],
...
PLUGINS = {
    ...
    "granite_docling": GraniteDoclingPlugin,
    "smoldocling": SmolDoclingPlugin,
}
